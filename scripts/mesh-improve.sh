...
case "$SUBCMD" in
  ...
  report)
    ...
    dolt sql -q "INSERT INTO mesh_improvements (id, gt_id, category, severity, command, title, description, proposed_fix, status, votes, created_at, updated_at)
      VALUES ('$IMP_ID', '$GT_ID', '$CATEGORY', '$SEVERITY', '$COMMAND', '$TITLE_ESC', '$DESC_ESC', '$FIX_ESC', 'reported', 1, NOW(), NOW());" 2>/dev/null
  ...
  graduate)
    ...
    IMP_TITLE=$(dolt sql -q "SELECT title FROM mesh_improvements WHERE id = '$IMP_ID';" -r csv 2>/dev/null | tail -n +2 | head -1 | sed 's/^"//;s/"$//')
    if [ -z "$IMP_TITLE" ]; then
      echo "[error] Improvement '$IMP_ID' not found"
      exit 1
    fi

    IMP_CAT=$(dolt sql -q "SELECT category FROM mesh_improvements WHERE id = '$IMP_ID';" -r csv 2>/dev/null | tail -n +2 | head -1)
    IMP_DESC=$(dolt sql -q "SELECT COALESCE(description, '') FROM mesh_improvements WHERE id = '$IMP_ID';" -r csv 2>/dev/null | tail -n +2 | head -1 | sed 's/^"//;s/"$//')
    IMP_FIX=$(dolt sql -q "SELECT COALESCE(proposed_fix, '') FROM mesh_improvements WHERE id = '$IMP_ID';" -r csv 2>/dev/null | tail -n +2 | head -1 | sed 's/^"//;s/"$//')

    # Build knowledge content
    KNOWLEDGE_CONTENT="### $IMP_TITLE"
...
**Fix:** $IMP_FIX"
    ...
    IMP_TITLE_ESC=$(echo "$IMP_TITLE" | sed "s/'/''/g")
    ...
    dolt sql -q "INSERT INTO mesh_knowledge_entries (id, category, title, content, source_improvement_id, contributed_by, created_at, updated_at)
      VALUES ('$K_ID', '$IMP_CAT', '$IMP_TITLE_ESC', '$KNOWLEDGE_CONTENT_ESC', '$IMP_ID', '$GT_ID', NOW(), NOW());" 2>/dev/null

    # Update improvement status
    dolt sql -q "UPDATE mesh_improvements SET status = 'published', fixed_by = '$GT_ID', updated_at = NOW() WHERE id = '$IMP_ID';" 2>/dev/null
